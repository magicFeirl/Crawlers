...
用 pyinstaller 转可执行文件结果因为没有创建子进程的权限又上网搜了半天，解决方法如下：
...
1. 使用异步 wxPython 轮子 wxasync 代替了多进程下的残废程序；

2. 实现代理端口支持;

**一些吐槽**

晚上请求接口速度慢的跟断网了似的，还以为是不是协程有问题，仔细一看发现设置代理端口的功能还是个摆设，于是实现了设置代理端口的功能，结果速度还是慢的跟蜗牛似的？。

9号找了一天 wxPython 陪和协程的方法，中文检索输入：wxPython 和 协程，完全没有想要的结果。

今天 Google 检索 wxPython and Coroutine 瞬间找到结果，反差巨大...

那么也有继续学 wxPython 的动力了，搞点简单的 GUI 程序还是挺好玩的（）。
